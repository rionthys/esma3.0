  ...
  <section class="content">
    <div class="header-details">
      ...
      <div class="details">
        ...
        <div class="desctiption">{{ affair?.number }}</div>
        <div class="desctiption">{{ affair?.view }}</div>
        <div class="desctiption">{{ affair?.number?.replace(/[^a-zA-Zа-яА-Я]/g, "") }}</div>
        <div class="desctiption">{{ affair?.status }}</div>
      </div>
    </div>
    <BasicWrapper>
      <table class="database__table">
        ...
        <tr v-for="(row, rowIndex) in filteredRows" :key="rowIndex" class="affairs-tr_table">
          ...
          <td class="affairs-details-table__row" @click="showDoc(row?.documents)">
            {{ row?.documents?.length > 0 ? row.documents.join('\n') : '-' }}
          </td>
          <td class="affairs-details-table__row">
    ...
    <CreateLogAffairs v-model:isVisible="showModalAction" @add-log="createAction()"/>
    <Modal v-model:isVisible="showModalResponsible">
      <div class="modal-close" @click="closeCreate"><img alt="" src="../assets/icons/cancel.png"></div>
      <div class="modal__content-title">Выберите исполнителя:</div>
      <div class="filter__content">
        <div v-for="user in users" :key="user.id"
             @click="changeResponsible(user)">
          {{ user.name }}
        </div>
      </div>
    </Modal>
    ...
    <div v-for="row in rows" :key="row.id"
         :class="['fixed', {'d-none': !row.documents || (row.documents && row.documents.length === 0) || row.documents !== pdfSrc}]"
      ...
      <IframeModal v-model:isVisible="showPdf">
        ...
        <div style="width: 700px; height: 470px">
          <iframe :key="`${pdfSrc}`"
                  :src="`https://docs.google.com/gview?url=https://echo-cloud.store/upload/${ row?.documents ? row?.documents[0] : ''}&embedded=true&time=${new Date().getTime()}`"
                  frameborder='0'
...
import CreateLogAffairs from "@/Blocks/CreateLogAffairs.vue";
...
export default {
  ...
  methods: {
    ...
    loadLogs() {
      AffairsApi.getLogs(this.$route.params.id).then((response) => {
        if (response) {
          this.rows = response.data.map(row => {
            console.log(row);
            return {
              ...row, documents: row?.documents ? row.documents.map((doc) => doc.replace('/', '')) : []
            }
          });
        }
      })
    ...
    changeResponsible(user) {
      AffairsApi.update(this.$route.params.id, {responsible: user.id, user: UsersApi.id}).then(
          (response) => {
            if (response.data) {
              this.showModalResponsible = false;
            }
            this.loadLogs();
          }
      );
    ...
    showResponsible() {
      this.showModalResponsible = true;
    },
    closeAffair() {
      AffairsApi.close(this.$route.params.id, {user: UsersApi.id}).then((response) => {
        if (response.data.id !== undefined) {
          this.affair.status = 'завершено';
          this.loadLogs();
        }
      })
    },
  ...
  mounted() {
    this.loadLogs();
    AffairsApi.getById(this.$route.params.id).then((response) => {
      if (response) {
