<template>
  <section class="content">
    <div class="header-details">
      ...
      <div class="details">
        <div class="desctiption">{{ contract?.number }} {{ contract?.client?.name }} до
    ...
    <BasicWrapper>
      <table class="database__table">
        ...
        <tr v-for="(row, rowIndex) in filteredRows" :key="rowIndex" class="affairs-tr_table">
          ...
          <td class="affairs-details-table__row" @click="showDoc(row.path)">
            {{ row?.path ? row.path : '-' }}
          </td>
          <td class="affairs-details-table__row">
    ...
    <CreateLogDocuments v-model:isVisible="showModalAction" @add-log="createAction()"/>
    <Modal v-model:isVisible="showModalResponsible">
      ...
      <div class="modal__content-title">Пожалуйста выберите нового исполнителя:</div>
    ...
    <div v-for="row in rows" :key="row.id" :class="[{'d-none': !row.path || row.path !== pdfSrc}]"
      ...
      <IframeModal v-model:isVisible="showPdf">
        ...
        <div style="width: 700px; height: 470px">
          <iframe :key="`${pdfSrc}`"
                  :src="`https://docs.google.com/gview?url=https://echo-cloud.store/upload/${row.path}&embedded=true&time=${new Date().getTime()}`"
                  frameborder='0'
...
import CreateLogDocuments from "@/Blocks/CreateLogDocuments.vue";
...
export default {
  ...
  methods: {
    ...
    loadLogs() {
      DocumentsApi.getLogs(this.$route.params.id).then((response) => {
        if (response) {
    ...
    changeResponsible(user) {
      AffairsApi.update(this.$route.params.id, {responsible: user.id}).then(
          (response) => {
            if (response.data) {
    ...
    navigateToAffairs() {
      this.$router.push(`/affairs/documents/${this.$route.params.id}`);
    },
    closeCreate() {
  ...
  mounted() {
    this.loadLogs();
    DocumentsApi.getById(this.$route.params.id).then((response) => {
      if (response) {
