  ...
  watch: {
    ...
    async allObjectsSelected(newVal) {
      if (newVal.selected) {
        ...
        if (newVal.data.path.name.split('.').length > 1) {
          newVal.data.path = (await UploadApi.upload(newVal.data.path)).data.fileName;
        }
        DocumentsApi.createLogs(this.$route.params.id, {...newVal.data, responsible: UsersApi.id}).then((affair) => {
