...
export default {
  ...
  watch: {
    ...
    async allObjectsSelected(newVal) {
      if (newVal.selected) {
        // console.log(newVal.data);x
        if (newVal.data.documents.name.split('.').length > 1) {
          newVal.data.documents = [(await UploadApi.upload(newVal.data.documents)).data.fileName];
        }
        AffairsApi.createLogs(this.$route.params.id, {...newVal.data, responsible: UsersApi.id}).then((affair) => {
